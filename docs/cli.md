...
- **`enroll`** — enroll a worker into a project with allowed CLIs (`--cli`, a
  ...
  here at once, still bounded by the project's Maximum Concurrent Jobs. There is
...
| Script | Description |
| --- | --- |
| `npm run dev:api` | Migrate the DB, free `API_PORT`, then start the API server (`:3101`) with `--watch`. In dev it serves the API only; it also serves the built dashboard SPA from `dashboard/dist` when that exists. It also runs the control-plane host maintenance loop — the orphaned-run reap and the worktree retention sweep (`src/api/maintenance.ts`, issue #550). |
...
| `npm run reload:all` | `reload` plus the step it only reminds you about: `docker compose up -d --build --wait router`. The router image *bakes* `dist` (`Dockerfile.router` — no source mount), so a pulled change is invisible to the control-plane dispatcher until it is rebuilt; `--wait` blocks on the service healthcheck, so a router that comes back broken fails the command instead of passing silently. Migrations still run before the rebuild, so the API is never left reading an older schema than the router — which applies them again itself at startup (`src/router/index.ts`). Still does **not** restart workers: they run phase code from their own checkout on their own machines, remote ones included. |
| `npm run reload:base` | The steps `reload` and `reload:all` share (deps, dashboard, migrations), with no closing reminder. Not meant to be run directly — it exists so the two reminders can differ without duplicating the chain. |
| `npm run dev:worker` | Start the worker (`src/transport/connect-entry.ts`). **This is the worker** — the only one, on every machine, remote or the control-plane host itself, where it points `SWARM_CONTROL_PLANE_URL` at the router over loopback (issue #551). It is not in Docker Compose, holds no `DATABASE_URL`/`REDIS_URL`, and needs `SWARM_WORKER_CREDENTIAL` and `SWARM_CONTROL_PLANE_URL` in `.env` — no operator SCM credential since issue #765, which arrives per assignment (`swarm workers set-scm-credential`). Besides running phases it reports **this machine's** agent-CLI quota to the control plane (issue #825) — shortly after startup, then every six hours — which is what fills the dashboard's CLI Quotas page for this worker's owner; a failed probe or report is logged and never stops the daemon. On the machine that *registered* the worker, [`swarm run:worker`](#swarm-runworker) starts this same script with both host-local values filled in from the per-checkout credential cache, so nothing has to be pasted (issue #788); this explicit form stays the path for a remote machine or a process supervisor. |
