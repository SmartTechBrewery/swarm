 ...
 * — antigravity's (issue #892) or codex's (issue #893) — rather than as an empty
 * Model selector offering a model that cannot launch. Other values pass through
 * untouched.
 *
 * A target that omits `cli` while naming a value only one non-default catalog
 * claims reads as that CLI, mirroring the pin `AgentTargetSchema` applies on
 * parse (`pinnedCliForModel`) — without it the CLI selector would show nothing
 * chosen and the Model selector would be disabled, for a phase the worker in fact
 * runs elsewhere.
 */
function normalizeTarget(target: AgentTarget): AgentTarget {
	if (!target.model) return { ...target };
	const cli = target.cli ?? pinnedCliForModel(target.model);
