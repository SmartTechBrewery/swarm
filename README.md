...
| **Respond-to-CI** | A check fails on a task branch | Fixes the failure, or reports that it can't | Commits/pushes and explains on the PR, under a per-PR attempt cap, which records a durable give-up when it is spent. Reporting that it can't fix the build hands the PR back to Review rather than ending the line |
