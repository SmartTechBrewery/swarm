...
- **Scope credentials with `AsyncLocalStorage`, never pass tokens as plain function arguments through multiple call layers.** Cascade's `withGitHubToken(token, fn)` / `withTrelloCredentials(creds, fn)` pattern keeps secrets out of function signatures and stack traces. Mirror this for every credential SWARM handles (GitHub implementer/reviewer tokens, GitHub Projects access).
  - **The one exception is the dashboard's pre-project SCM API, and it stays one layer deep.** Those reads (`src/api/routers/scm.ts` — the per-provider `verify…` procedures and `scm.defaultBranch`) run *before* a project exists to scope a credential from — for the default-branch read the project is the very thing being created — so the procedure may pass an already-resolved instance credential as an argument to exactly one provider-dispatch helper (`src/api/scm-verification.ts`, `src/api/scm-default-branch.ts`), which hands it straight to that provider's client, where it is scoped on entry (`withGitLabToken` / `withBitbucketCredential`, or bound to one locally constructed Octokit). It travels no further, is never logged or returned, and every project-scoped flow keeps using `AsyncLocalStorage`. See ai/RULES.md §2 "Source-control features must not hard-code GitHub" for why that route is deliberately concrete rather than generalised over the registry.
...
- **Drop SWARM-generated content by its marker.** Every comment SWARM posts carries a hidden `<!-- swarm-… -->` marker or the visible `_Generated by SWARM…_` footer; one provider-neutral predicate (`isSwarmGeneratedBody`, `src/scm/swarm-origin.ts`) tests for them, and the router's comment drop gate (`SCMProvider.isSwarmGeneratedEvent`) is built on it. Every new SWARM comment body must append one of the two — the gate is only as good as that coverage. Same idea for "is this PR ours": the PR-ownership triggers — `pr-review` and `resolve-conflicts` — decide it from the PR's work-item origin (`isSwarmManagedPullRequest`, shared by both since issue #836), not its author.
...
Drop gates stay narrowly scoped: SWARM's comments are the runaway loop, so the comment gate drops them, while PR/review/check lifecycle events must flow through even when SWARM produced them (that is how one persona hands off to the other). The one remaining identity-based drop is the PM board's status-change gate (`PMRouterAdapter.isSelfAuthored`) — a status change carries no body to mark, so it has nothing to inspect. That gate is a **per-provider** obligation with an explicit contract (`src/pm/router-adapter.ts`, ai/RULES.md §2): each provider establishes the identity itself rather than borrowing another category's, and fails *open* on a resolution failure so a real human status change is never silently dropped as "ours".
