...
Today SWARM's only *implemented* PM provider is **GitHub Projects**, but Jira, Linear, and Trello connectors are planned (Cascade already ships all three — it's the structural precedent per §1) and `PMType` / `PmProviderIdSchema` already name all four as vocabulary (issue #297, exactly as `ScmType` named `bitbucket`/`gitlab` before either registered). **Every feature that touches the project board must be built behind the provider-agnostic contract — `PMProvider` (`src/pm/types.ts`) outbound, `PMRouterAdapter` (`src/pm/router-adapter.ts`) inbound — never against GitHub Projects directly**, so a new connector drops in by implementing those interfaces alone — no pipeline, trigger, or phase code changes.
...
- **Program against the contract, not the concrete provider.** Pipeline phases, triggers, and the worker take a `PMProvider` (or have one injected — `ctx.pm` on a PM trigger context) and call only its interface methods. They must not import `createGitHubProjectsProvider` or reach into `src/integrations/pm/github-projects/`: since issue #297 every project-scoped call site resolves through `requireProjectPMProvider(project)` / `requireProjectPMAdapter(project)` (`src/integrations/pm/registry.ts`, re-exported from the `src/integrations/pm/index.ts` barrel), and no pipeline, worker, or router module constructs a provider.
  - Those two lookups are a **plain registry lookup on `project.pm.type`**, with none of the SCM side's single-provider assertion or `runtimeReady` flag: `project.pm.type` already *is* the config discriminator `ProjectConfig` lacks on the SCM side, so project→provider selection is already designed. Don't copy `requireProjectSCMProvider`'s assertion pattern here.
...
- **Keep provider-specific shapes out of shared code.** No GitHub-issue-URL parsing, `projects_v2` payload assumptions, or GraphQL node IDs leaking into pipeline/trigger logic. When a piece of data is provider-specific, resolve it through a `PMProvider` method or the work item's generic fields (`id`, `url`, `status`), not by pattern-matching a GitHub shape. (Example: `src/pipeline/respond-to-review.ts` resolves its board card via `listWorkItems()` + the generic `url` field, so it works unchanged for any provider.) Inbound is the same: the durable queue envelope is `{ type: 'pm', providerId, event }` carrying a normalized, provider-neutral `PmEvent` (`src/pm/events.ts`), so raw webhook event names, payload paths, and board field ids stop at the adapter.
- **Loop prevention is a per-provider obligation.** `PMRouterAdapter.isSelfAuthored` answers "did SWARM itself cause this board change?" using an identity the *PM provider* can establish on its own. A provider MUST NOT borrow another category's identity model: GitHub Projects may keep reaching into `GitHubSCMIntegration`'s persona helpers only because the board and the repo are the same account (named below as one of three deliberate reaches); a Jira board paired with a GitHub repo has no such shared identity and must key on its own bot account / API-token identity. Fail **open** (return `false` and log) on any identity-resolution failure — a swallowed error must never silently drop a real human status change.
- **If the interface is too small for a feature, widen the interface** (add a method to `PMProvider` and implement it for every provider), rather than special-casing GitHub Projects at the call site. Mirror Cascade's `src/pm/types.ts` when deciding the method shape.
...
Same story on the SCM side: **GitHub is the only source-control provider SWARM actually runs on today. Bitbucket (issue #296) and GitLab (issue #295) both implement the whole contract and neither is runtime-reachable.** The SCM side now has the same two-part shape as the PM side: a provider-neutral contract, **`SCMProvider` (`src/scm/types.ts`)**, which `GitHubSCMIntegration` (`src/integrations/scm/github/scm-integration.ts`) declares and satisfies, plus an SCM manifest/registry (`src/integrations/scm/{manifest,registry}.ts`) that each provider self-registers into from the single canonical entrypoint (`src/integrations/entrypoint.ts`). The contract composes the two seams that already existed — it extends `ScmMergeProvider` (`src/scm/merge.ts`) and returns `ScmDeliveryProvider` (`src/scm/delivery.ts`) — rather than duplicating them, and `ScmPersona`/`ScmPersonaIdentities` are the neutral names for the implementer/reviewer role model (`GitHubPersona`/`PersonaIdentities` remain as GitHub-local aliases).

**A complete provider still carries no traffic until something selects it — and two now sit there.** `BitbucketSCMIntegration` (`src/integrations/scm/bitbucket/`, issue #296 phase 4/4) and `GitLabSCMIntegration` (`src/integrations/scm/gitlab/`, issue #295 phase 4/4) each satisfy the **whole** `SCMProvider` contract: a credential-scoped client, persona credential resolution (implementer from a worker-local `SWARM_OPERATOR_BITBUCKET_TOKEN` / `SWARM_OPERATOR_GITLAB_TOKEN`, reviewer from the same provider-neutral `credentials.reviewer` reference GitHub uses), persona identity/loop-prevention helpers, webhook ingress, the pull-request/CI-status reads, and the writes: idempotent comments, review verdicts, the delivery seam (commit identity, PR find/create, expected-SHA push, review submission, idempotent comment), and the direct merge with its own outcome classification. Neither stubs a method, and the multi-provider conformance suite (`tests/unit/integrations/scm/scm-conformance.test.ts`) asserts that for every registered manifest. What both still lack is **wiring**, not implementation: each manifest declares `runtimeReady: false` (`src/integrations/scm/manifest.ts`), which is what keeps their registrations a no-op for GitHub — the project-scoped lookup counts only runtime-ready manifests, and the receiver mounts a webhook route only for them. That flag is **not** provider selection and does not soften the single-provider assertion below: the second provider to claim runtime readiness still lands as a throw, so selection gets designed then. Flipping either needs project→provider selection **plus** a served ingress route — one separate follow-up issue that both providers are now waiting on (see the closing paragraph), not something a phase of #296 or #295 smuggles in.
...
Don't write GitHub assumptions into places another provider would then have to unpick. **Neither a Bitbucket nor a GitLab project can run end to end**: both implement every contract method now that all four phases of #296 and #295 have landed, but nothing selects a project's SCM provider — that needs project→provider selection plus a served ingress route, a separate follow-up issue on top of the inbound/outbound migrations #385/#386 already did, not something a phase of either issue smuggles in. Two finished providers now wait on that one piece of wiring, which is the argument for designing it properly rather than for either issue having smuggled it in.
