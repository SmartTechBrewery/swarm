...
Same story on the SCM side: **GitHub is the only source-control provider SWARM actually runs on today; Bitbucket is being built out phase by phase (issue #296) and GitLab is still planned.** The SCM side now has the same two-part shape as the PM side: a provider-neutral contract, **`SCMProvider` (`src/scm/types.ts`)**, which `GitHubSCMIntegration` (`src/integrations/scm/github/scm-integration.ts`) declares and satisfies, plus an SCM manifest/registry (`src/integrations/scm/{manifest,registry}.ts`) that each provider self-registers into from the single canonical entrypoint (`src/integrations/entrypoint.ts`). The contract composes the two seams that already existed — it extends `ScmMergeProvider` (`src/scm/merge.ts`) and returns `ScmDeliveryProvider` (`src/scm/delivery.ts`) — rather than duplicating them, and `ScmPersona`/`ScmPersonaIdentities` are the neutral names for the implementer/reviewer role model (`GitHubPersona`/`PersonaIdentities` remain as GitHub-local aliases).

**A provider under construction registers, but carries no traffic.** `BitbucketSCMIntegration` (`src/integrations/scm/bitbucket/`) is registered and resolvable by id, with a credential-scoped Bitbucket Cloud client, persona credential resolution (implementer from the worker-local `SWARM_OPERATOR_BITBUCKET_TOKEN`, reviewer from the same provider-neutral `credentials.reviewer` reference GitHub uses), persona identity/loop-prevention helpers, webhook ingress, and the pull-request/build-status reads; every contract method it hasn't built yet — comments, delivery, merge — **throws a message naming the method and the phase that fills it in** rather than returning a misleading empty value. Its manifest declares `runtimeReady: false` (`src/integrations/scm/manifest.ts`), which is what keeps that registration a no-op for GitHub: the project-scoped lookup counts only runtime-ready manifests, and the receiver mounts a webhook route only for them. That flag is **not** provider selection and does not soften the single-provider assertion below — the second provider to claim runtime readiness still lands as a throw, so selection gets designed then. Do not flip it for a provider whose contract still throws.
...
As above, don't *build* a GitLab provider until it's needed — just don't write GitHub assumptions into places a second provider would then have to unpick. **A Bitbucket project still cannot run end to end**, even once #296's later phases land: nothing selects a project's SCM provider, so that needs project→provider selection plus a served ingress route — a separate follow-up, not something a phase of #296 smuggles in.
