...
**A provider under construction registers, but carries no traffic.** `BitbucketSCMIntegration` (`src/integrations/scm/bitbucket/`) is registered and resolvable by id, with a credential-scoped Bitbucket Cloud client, persona credential resolution (implementer from the worker-local `SWARM_OPERATOR_BITBUCKET_TOKEN`, reviewer from the same provider-neutral `credentials.reviewer` reference GitHub uses), and persona identity/loop-prevention helpers; every contract method it hasn't built yet **throws a message naming the method and the phase that fills it in** rather than returning a misleading empty value. Its manifest declares `runtimeReady: false` (`src/integrations/scm/manifest.ts`), which is what keeps that registration a no-op for GitHub: the project-scoped lookup counts only runtime-ready manifests, and the receiver mounts a webhook route only for them. That flag is **not** provider selection and does not soften the single-provider assertion below — the second provider to claim runtime readiness still lands as a throw, so selection gets designed then. Do not flip it for a provider whose contract still throws.
