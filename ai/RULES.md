...
Same story on the SCM side: **GitHub is SWARM's only source-control provider today, but Bitbucket and GitLab are planned.** The SCM side now has the same two-part shape as the PM side: a provider-neutral contract, **`SCMProvider` (`src/scm/types.ts`)**, which `GitHubSCMIntegration` (`src/integrations/scm/github/scm-integration.ts`) declares and satisfies, plus an SCM manifest/registry (`src/integrations/scm/{manifest,registry}.ts`) that GitHub self-registers into from the single canonical entrypoint (`src/integrations/entrypoint.ts`). The contract composes the two seams that already existed — it extends `ScmMergeProvider` (`src/scm/merge.ts`) and returns `ScmDeliveryProvider` (`src/scm/delivery.ts`) — rather than duplicating them, and `ScmPersona`/`ScmPersonaIdentities` are the neutral names for the implementer/reviewer role model (`GitHubPersona`/`PersonaIdentities` remain as GitHub-local aliases).
...
- **`requireProjectSCMProvider(project)`** for a project-scoped operation with no event in hand. It performs **no selection and no fallback**: it asserts that exactly one provider is registered and throws otherwise. `ProjectConfig` has no provider discriminator, so that assertion is the honest statement of today's invariant — and the throw is the forcing function that makes the *second* provider's author design project→provider selection (a config discriminator, a repo-URL match, whatever fits) instead of inheriting a silent "whichever registered first" pick. Registering a second manifest without that design is expected to fail loudly.
...
- **Live board**: <https://github.com/orgs/SmartTechBrewery/projects/6/views/1> — owner `SmartTechBrewery` (org-level project), project number `6`, project (node) id `PVT_kwDODb1Ycc4Bcnwu`. Org-owned so a plain org webhook can deliver `projects_v2_item`. Currently holds only the `phase-6` dashboard issues (#75–86), copied over from project `3` as the MVP dogfooding test set — it is not (yet) the full backlog. The repo now lives under the same org (`SmartTechBrewery/swarm`), which keeps the issue-sidebar Projects picker working; a project's owner and its items' repo don't strictly need to match, but a cross-owner pairing means issues can only be added to the board by pasting their URL or via `gh project item-add`.
- **Repo**: `SmartTechBrewery/swarm` (transferred from `jkwiecien/swarm` on 2026-08-04; the old path still redirects, but nothing should reference it — the router matches inbound webhooks on the exact `repository.full_name`, so a stale `repo` value silently drops every SCM event). Every task is a GitHub issue in this repo. New/active work gets added to project `6` (live); project `3` is not actively maintained going forward.
