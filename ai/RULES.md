...
**The migration is not done.** Ingress (webhook receiver, `GitHubRouterAdapter`, the queue's GitHub-shaped job payload) and the pipeline/worker/transport call sites still import GitHub concretely — that is issue #385 (ingress, including the provider-neutral webhook *event* model) and issue #386 (pipeline/worker). So resolving a provider through the registry is available but not yet how anything runs, and the rule below still governs every SCM feature you write:
