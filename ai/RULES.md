...
**A provider still being built registers nothing at all.** `GitLabSCMIntegration` (`src/integrations/scm/gitlab/`) is landing over four phases along the same seams Bitbucket used (issue #295). Phase 1/4 built the persona-scoping half — a credential-scoped GitLab.com REST v4 client, persona credential resolution (implementer from the worker-local `SWARM_OPERATOR_GITLAB_TOKEN`, reviewer from the same provider-neutral `credentials.reviewer` reference), and persona identity / loop-prevention helpers. Phase 2/4 added the inbound half in `gitlab/webhook.ts` (header interpretation, the timing-safe `X-Gitlab-Token` comparison that stands in for an HMAC because GitLab's own mechanism is a shared token, merge-request / note / pipeline normalization, and the comment loop-prevention gate) plus the `gitlab/pipelines.ts` status vocabulary the phase-3 reads share. Phase 3/4 added the merge-request and commit-status reads in `gitlab/merge-requests.ts`: the real `mergeable` tri-state from `merge_status` with a `detailed_merge_status` fallback, `diff_refs.base_sha` as the merge base, a per-candidate re-read for conflict candidates, and commit statuses reusing `pipelines.ts` unchanged. Only the four phase-4 writes still throw a message naming the method **and** its phase. Where Bitbucket registered a `runtimeReady: false` manifest from its own phase 1, GitLab deliberately **does not register at all** until its final phase: the conformance suite asserts that no *registered* provider stubs a contract method, and `ai/TESTING.md` frames that assertion as the gate on the third provider, so registering a stub-bearing manifest would force an exemption that disables the gate for the two providers already passing it. Until phase 4/4 nothing resolves `gitlab` by id — the only shared-surface change is `ScmType` / `ScmProviderIdSchema` gaining the value — and every test constructs the class directly. Prefer that shape for the next provider too: register when the contract is satisfied, not when the folder appears.
...
Don't write GitHub assumptions into places another provider would then have to unpick. **Neither a Bitbucket nor a GitLab project can run end to end**: Bitbucket implements every contract method now that all four of #296's phases have landed, and GitLab is filling its own in under #295, but nothing selects a project's SCM provider — that needs project→provider selection plus a served ingress route, a separate follow-up issue on top of the inbound/outbound migrations #385/#386 already did, not something a phase of either issue smuggles in.
