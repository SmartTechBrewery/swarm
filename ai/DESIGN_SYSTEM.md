...
**Whole-card link (a card that navigates)** — a card whose only job is to open one thing is a link, and the *whole* card is the target: the name text alone is a target small enough that operators don't read it as a link at all (issue #781). Keep the existing name `<Link>` and stretch it with an `::after` overlay — `relative` (plus `group`) on the card, `after:absolute after:inset-0 after:rounded-lg focus:outline-none` on the link — rather than wrapping the card's contents in the anchor, which would make the link's accessible name a dump of every fact on the card and nest anything interactive inside an anchor. The affordance goes on the card, not the name: `cursor-pointer hover:bg-zinc-800/40 hover:border-zinc-700 focus-within:border-violet-500 focus-within:ring-1 focus-within:ring-violet-500`, with the name following via `group-hover:`. Anything inside the card that means something of its own — a badge with a `title`, later an in-card control — sits **above** the overlay (`relative z-10`): a covered element's `title` never resolves (a browser looks *up* from what the pointer is over, never down at what it covers), and that layer is where an in-card button belongs, above the link rather than nested in it. `MyWorkersPanel`'s `WorkerCard` is the reference. Use this where the card is a link; use the responsive-list pattern below where the card mirrors a clickable table row that already carries in-card links.

**Responsive list (table → cards on mobile)** — a wide multi-column table (roughly 5+ columns) does not fit a ~375–430px phone; horizontally scrolling it is not a mobile-native list. Instead render both presentations from the same data: the `<table>` above inside a `hidden md:block` wrapper, and a `md:hidden` stacked card list (`space-y-3`), one card per row. Each card is a nested sub-panel (`border border-zinc-800 rounded-lg bg-panel/20 p-4 shadow-sm`, generous spacing between cards for tap targets) with a deliberate hierarchy, **not** a uniform field dump: a **primary line** (the row's identifying title as the dominant text, the status badge on the same line), a lighter **secondary line** of supporting metadata (`text-xs text-zinc-400`), and a subordinate **footer** of reference details (`text-xs text-zinc-500`). No field is dropped — de-emphasized is fine, silently truncated is not; let long values wrap (`break-words`/`break-all`, `min-w-0`) so nothing forces horizontal scroll. A whole-card tap navigates like the desktop row (`role="button"`, `tabIndex={0}`, Enter/Space handling); in-card links/buttons call `stopPropagation` (or sit in a separated action row) so they don't trigger card navigation. The run lists (`RunsTable`, `QueuedRunsSection`) use this pattern; the shared desktop column widths live in `dashboard/src/lib/run-table-layout.ts`.

**Describing a run's work** — never label a run by its id: a UUID tells an operator nothing. Render `WorkItemCell` (`dashboard/src/components/runs/work-item-cell.tsx`) — the resolved Issue/PR title as the primary line over its `Issue: #n` / `PR #n` provider reference — so every screen describes the same run identically (issue #473). Pass `titleHref` where the surrounding row isn't already clickable (the Workers table's **Active job** column) and the title becomes the link to the run; omit it where the row navigates on click, so the cell doesn't swallow it. Pass `phaseLabel` only where the surface has no Phase column of its own — it adds a leading line above the title rather than crowding it — and never alongside one, which would say the same thing twice in the same row.

**Fixed-width columns** — a table whose cells hold prose needs `table-fixed` plus a `<colgroup>` of percentage widths, so `truncate` actually truncates and one long title can't push the row sideways. Give the prose column the bulk of the width and size the rest to their own content (`RunsTable`, `WorkersTable`). A table like that is the page's content, not a card in it: don't cap the route in a `max-w-*` wrapper and leave a gutter — the freed width belongs to the prose column.
...
**Danger card (an irreversible delete)** — a destructive, cascading delete is its own card at the *end* of the screen it belongs to, never a button among the fields of a form that saves something else. One shape, two instances so far: `workers/worker-delete-card.tsx` (issue #789) and `projects/project-delete-card.tsx` (issue #854) — copy theirs rather than inventing a third look. A short paragraph saying when to use it, then an **entry button** (`inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-red-200 bg-red-950/40 border border-red-900/50 rounded-md hover:bg-red-900/40 focus:outline-none focus:ring-1 focus:ring-red-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed`) that only *opens* a `Modal` — it never deletes. The modal names the thing being deleted, lists what the delete cascades to and what survives it, and its **confirm button** is the filled danger variant (`bg-red-600 hover:bg-red-500 focus:ring-red-500`, otherwise the primary-button geometry) beside a plain secondary Cancel. Where the delete destroys history the confirm stays `disabled` until the operator types the entity's id back into a `font-mono` field. The server's refusal is rendered verbatim in the error banner *inside* the modal, which stays open — the server, not the card, decides whether a delete is allowed.

**Status dot** (e.g. daemon connection) — `h-2 w-2 rounded-full bg-emerald-500 ring-4 ring-emerald-500/10`.

**Meta pill/badge** (e.g. version tag) — there is exactly one: `Badge` (`dashboard/src/components/ui/badge.tsx`), geometry `px-2 py-0.5 text-[10px] uppercase font-mono font-bold tracking-wider rounded border`, neutral hue `bg-zinc-850 text-zinc-400 border-zinc-800`. Render it for every short stamped label; never hand-roll a second pill with its own size or weight. Only the **hue** varies, via its `tone`: `positive`/`caution`/`negative` carry state (the worker detail view's enrollment approval status and its routable/not-routable verdict) — `caution` (`bg-amber-950/20 text-amber-200 border-amber-900/30`) also makes the `planning` badge read ahead of the others wherever it appears (the `/workers` Capabilities cell, the worker detail view's declared and allowed pipeline phases), the same amber whether or not that instance of `planning` is otherwise flagged unavailable. `caution` also carries the runs surfaces' `Maintenance` mark (`dashboard/src/components/runs/maintenance-run-badge.tsx`, issue #974) — the one place the hue marks a run's *kind* rather than its status, which is not the #542 mistake repeated: it marks the row that is **not** a member of the set of pipeline runs, and the state it carries is that none of the pipeline recovery actions apply to it. It sits beside `RunStatusBadge`, never instead of it, so the kind and the status stay two readable axes. Same geometry, same spacing, different weight of voice; never resize a badge to emphasize it.

**Pipeline-phase order** — wherever a screen lists pipeline phases (the worker detail view's declared and allowed phases, the Runs Phase filter), the sequence is the pipeline's own — `planning → implementation → review → respond to review → respond to ci → resolve conflicts` — via `sortPipelinePhases` (`dashboard/src/lib/pipeline-phases.ts`) for a list a daemon or read model handed over, or a map over `ALL_TRIGGER_PHASES` (`src/triggers/types.ts`) when the list *is* the whole vocabulary; never the order a value happened to arrive in, and never a hand-written sequence, so a seventh phase orders itself everywhere at once (issue #548).

**Checkbox** — `h-4 w-4 rounded border-zinc-700 bg-zinc-900 text-violet-600 focus:ring-1 focus:ring-violet-500 disabled:opacity-50 disabled:cursor-not-allowed`, inside a `<label className="inline-flex items-center gap-2 text-sm text-zinc-300">` so the text is the accessible name (add `font-mono` to that text for an identifier, e.g. a CLI name). Use it for a *set* of independent booleans where a row of switches would read as unrelated toggles (the worker detail view's allowed agent CLIs); a single boolean stays a `ToggleSwitch`. When one box may not be unchecked because the server would reject the result (the last allowed CLI), render it `disabled` with a `title` saying why — same rule as a switch the viewer may see but not change.
...
- **The connection state is an icon, not a sentence.** The footer is an account area, so a small state icon carries the connection status on its own — a distinct shape per state (`CircleCheck`/`CircleX`/`CircleDashed`), never color alone, so the state reads without relying on color perception or a pointer hover. Wrap it in `role="status"` with an `aria-label` (mirrored in `title`) so it has an accessible, live-announced name without needing a focusable control; don't reintroduce a "Connected"/"Disconnected" text label beside it. **It trails the row — the leading icon belongs to the account entry (issue #680).** A state icon in front of the user's name is read as that entry's own icon, and a `CircleCheck` there says nothing about profile navigation, so the profile link leads with a plain outlined `User` bust (`h-3.5 w-3.5`, `aria-hidden` — the visible display name stays the link's accessible name) and the status icon sits at the far end of the row.
- Main content: `main.flex-1 overflow-y-auto` with an inner `p-4 md:p-8` padding wrapper around the route's `Outlet` (`dashboard/src/routes/__root.tsx`) — there is no centering wrapper at the shell level; a screen that wants a centered column adds its own `max-w-* mx-auto`.
