...
**Meta pill/badge** (e.g. version tag) — `px-2 py-0.5 text-[10px] uppercase font-mono font-bold tracking-wider bg-zinc-850 text-zinc-400 rounded border border-zinc-800`. To make **one** badge in a set read ahead of the others, keep every other class and swap only the hue to the violet accent — `bg-violet-950/30 text-violet-300 border-violet-900/30` (the `/workers` Capabilities cell does this for `planning`). Same geometry, same spacing, different weight of voice; never resize a badge to emphasize it.
