/**
 * Bitbucket Cloud's commit vocabulary, shared by the two halves that have to
 * agree on it: webhook ingress (`./webhook.ts`, phase 2/4) and the REST reads
 * (`./pull-requests.ts`, phase 3/4). Both a normalized event's `headSha` and an
 * API read's `headSha` end up in the *same* exact-match consumers — the
 * review-dispatch dedup key (`src/triggers/review-dispatch-dedup.ts`), the
 * `reviewVerdicts.headSha` SQL predicate, the conflict-resolution claim key — so
 * a second spelling of either the commit hash or a build state would silently
