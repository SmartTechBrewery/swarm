 ...
 * a `runs` row for every phase it dispatched. So a PR is SWARM-managed iff its
 ...
 *
 * The run row is the stronger half: it exists only because the `pm-status`
 * trigger already verified board membership *and* the `pipeline.automationLabel`
 * opt-in before dispatching Implementation, so a human PR that merely happens to
...
import { issueNumberFromBranch } from '../pipeline/respond-to-review.js';
 ...
 * definitive `false` and costs no query. A failing run-history lookup throws:
...
export async function isSwarmManagedPullRequest(
	...
	const taskId = headBranch ? issueNumberFromBranch(headBranch, project.branchPrefix) : undefined;
	...
	return hasRunForTask(project.id, taskId, 'implementation');
}
