...
export async function runRespondToCiPhase(
	...
	try {
		...
		// fix belongs on the newer tip and must not fail; a branch that no longer
		// contains the checked commit fails here, before an agent token is spent.
		...
		if (!deliveryResumed) {
			...
			if (held !== 'unchanged')
				logger.warn(
					'respond-to-ci: the PR branch advanced past the checked head — fixing on the newer tip',
					{
						taskId,
						prNumber,
						prBranch,
