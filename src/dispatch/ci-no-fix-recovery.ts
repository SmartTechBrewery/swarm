 ...
 * extends it to cover the fix's wall clock at the moment it dispatches — the
 * project's own effective `respond-to-ci` timeout plus margin
 * (`ciFixClaimTtlSec`, `src/triggers/handlers/review.ts`), so a configured
 * timeout is covered too rather than only the default one. That is what makes
 ...
 * extended lease lapsed — a Redis restart. There the per-PR fix-attempt cap
