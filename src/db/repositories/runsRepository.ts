...
/**
 * One worker-update run as a machine's own page reads it (issue #977) — the build it
 * was asked to move to, when it was asked, how it ended, and the machine's own words
 * when it failed.
 *
 * Narrower than {@link RunRow} on purpose: this crosses into a dashboard read model,
 * so it names exactly the columns that surface there and nothing else. `target` is
 * nullable because the column is (a pipeline run has none), never because a
 * worker-update run is written without one.
 */
export interface WorkerUpdateRunRow {
	runId: string;
	projectId: string;
	/** `runs.maintenance_target` — the build this request was moving the machine to. */
	target: string | null;
	status: string;
	/** When the machine was asked: a worker-update run starts at the request. */
	startedAt: Date;
	completedAt: Date | null;
	durationMs: number | null;
	/** The machine's own reason, for a `failed` run; null for one that completed. */
	error: string | null;
}

/**
 * How many of a machine's update runs its own page shows. Bounded rather than
 * paginated because the question that surface answers — when was this machine last
 * updated, and did it work — is answered by the newest few: a machine asked weekly
 * for a year is 52 rows nobody scrolls. The runs list, which is paginated and
 * filterable, stays where the whole history is read.
 */
export const WORKER_UPDATE_HISTORY_LIMIT = 10;

/**
 * One machine's most recent update runs, newest first (issue #977).
 *
 * `kind` is in the predicate **positively** rather than left implied by the
 * `worker_id` match: a second kind of machine maintenance would otherwise join a card
 * titled "Update history" the day it is first written, with nothing here edited to
 * let it in.
 *
 * Keyed on `worker_id`, the *live* link, so this needs no `maintenance_machine`
 * fallback: `ON DELETE SET NULL` detaches a retired machine's runs, and a retired
 * machine has no page for them to appear on. The runs list keeps that fallback
 * precisely because it is the surface where those rows do stay readable.
 */
export async function listWorkerUpdateRunsForWorker(
	workerId: string,
	limit = WORKER_UPDATE_HISTORY_LIMIT,
): Promise<WorkerUpdateRunRow[]> {
	return getDb()
		.select({
			runId: runs.id,
			projectId: runs.projectId,
			target: runs.maintenanceTarget,
			status: runs.status,
		...
		.from(runs)
		.where(and(eq(runs.kind, WORKER_UPDATE_RUN_KIND), eq(runs.workerId, workerId)))
